    ...
    window.setTimeout(function() {
        ...
        $.getJSON('daten/orte.json?=' + Date.now(), function(data) {
            ...
            $.each(data, function(orte, daten) {
                $('#uebersicht').find('ul').append('<li class="' + daten.Kategorie + '"><img src="http://maps.google.com/maps/api/staticmap?center=' + daten.Position.Latitude + ',' + daten.Position.Longitude + '&zoom=13&size=50x50&markers=color:blue|size:tiny|' + daten.Position.Latitude + ',' + daten.Position.Longitude + '&sensor=true"/>' + orte + '<span>' + entfernungBerechnen(meineLongitude, meineLatitude, daten.Position.Longitude, daten.Position.Latitude) + '</span></li>');
